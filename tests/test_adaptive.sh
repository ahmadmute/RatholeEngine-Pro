...
(
  ...
  result="$(adaptive_probe_tcp "127.0.0.1" "1" 2>/dev/null || true)"
  [[ "$result" =~ ^(healthy|tcp_timeout|tcp_refused|tcp_failed):[0-9]+$ ]] || fail "adaptive_probe_tcp natijeh-e namotabar: $result"
  marker="$ROOT/injection-ran"
  adaptive_probe_tcp "127.0.0.1;touch $marker" "1" >/dev/null 2>&1 || true
  [ ! -e "$marker" ] || fail 'adaptive_probe_tcp host ra be shell code tabdil kard'
  ok 'adaptive_probe_tcp output motabar ast va host injection ejra nemishavad'
)

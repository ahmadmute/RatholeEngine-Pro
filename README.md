...
- **hardened management plane** *(v1.5.1)* — atomic/locked state writes, PBKDF2 passwords, HttpOnly sessions, validated SSH targets, checksummed updates and traversal-safe root bundle extraction.

Details: [`docs/transport-modes.md`](docs/transport-modes.md).

## Quick start

**One command from GitHub** (downloads the latest release bundle, then runs the installer). Defaults to `ahmadmute/RatholeEngine-Pro` — override with `RATHOLE_GH` to use a fork:
...
> `install.sh` fetches `SHA256SUMS`, verifies the selected bundle and `bootstrap.sh`, then hands off to the verified bootstrap. Override the source repo with `RATHOLE_GH="you/repo"` or pin a version with `RATHOLE_RELEASE="v1.2.3"`. Unverified updates are rejected by default.

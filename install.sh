...
#   curl -fsSL https://raw.githubusercontent.com/ahmadmute/RatholeEngine-Pro/main/install.sh | sudo bash -s -- --rollback
#
...
#
# repo slug pishfarz ahmadmute/RatholeEngine-Pro ast؛ baraye estefade az fork-e khodet ba mtghir-e mohiti
# override kon:   RATHOLE_GH="youruser/yourrepo" curl -fsSL .../install.sh | sudo bash -s -- ...
set -euo pipefail

# ---------- tanzimat ----------
GH="${RATHOLE_GH:-ahmadmute/RatholeEngine-Pro}"    # owner/repo — pishfarz، ba RATHOLE_GH override kon
...
main(){
  ...
  # 3) verify-e release asset-ha. Release-haye ghadimi momken ast SHA256SUMS nadashte bashand؛
  # baraye Pro v1.5.1+ vojood-e checksum pishfarz ast.
  if fetch "$DL/SHA256SUMS" "$tmp/SHA256SUMS"; then
    command -v sha256sum >/dev/null 2>&1 || die "sha256sum baraye verify-e release lazem ast."
    local selected="$tmp/SHA256SUMS.selected" bname
    bname="$(basename "$bundle")"
    awk -v b="$bname" '$2=="bootstrap.sh" || $2==b {print}' "$tmp/SHA256SUMS" > "$selected"
    [ "$(wc -l < "$selected")" -eq 2 ] || die "SHA256SUMS entry-e bootstrap/bundle ra nadarad."
    ( cd "$tmp" && sha256sum -c "$(basename "$selected")" --strict ) || die "checksum-e release motabegh nist; nasb motevaghef shod."
    log "checksum-e bootstrap va bundle tayid shod."
  else
    warn "SHA256SUMS dar in release peyda nashod (release-e ghadimi). verify-e asset anjam nashod."
  fi

  # 4) tahvil be bootstrap.sh (hameye argvman-haye passed be bootstrap miravand)

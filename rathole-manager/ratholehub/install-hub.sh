...
if [ ! -f "$CONF_DIR/config.json" ]; then
  ...
  [ "${#PW}" -ge 10 ] || die "ramz panel bayad hadaghal 10 karakter bashad."
  PWHASH="$(printf '%s' "$PW" | python3 -c '
import sys,hashlib,secrets,base64
p=sys.stdin.buffer.read(); salt=secrets.token_bytes(16); rounds=310000
d=hashlib.pbkdf2_hmac("sha256",p,salt,rounds)
e=lambda b:base64.urlsafe_b64encode(b).decode().rstrip("=")
print(f"pbkdf2_sha256${rounds}${e(salt)}${e(d)}")
')"

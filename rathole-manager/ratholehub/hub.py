...
import os, sys, json, hmac, hashlib, time, subprocess, re, secrets, threading, shlex, shutil, base64
...
def hash_password(password):
    """PBKDF2-SHA256 with a per-password random salt (stdlib only)."""
    if isinstance(password, str):
        password = password.encode("utf-8")
    salt = secrets.token_bytes(16)
    digest = hashlib.pbkdf2_hmac("sha256", password, salt, _PBKDF2_ITERATIONS)
...
def password_value(cfg):
    return str(cfg.get("admin_password_hash") or cfg.get("admin_password_sha256") or "")
...
def verify_password(stored, password):
    ...
    try:
        if stored.startswith("pbkdf2_sha256$"):
            ...
            rounds_i = int(rounds)
            if rounds_i < 100000 or rounds_i > 2000000:
                return False
            expected = _b64decode_nopad(digest_s)
            actual = hashlib.pbkdf2_hmac("sha256", password, _b64decode_nopad(salt_s), rounds_i)
            return hmac.compare_digest(actual, expected)
        # compatibility with <=1.5.0; migrated after a successful login/change.
        if re.fullmatch(r"[0-9a-fA-F]{64}", stored or ""):
            return hmac.compare_digest(hashlib.sha256(password).hexdigest(), stored.lower())
    except Exception:
        return False
    return False

def password_is_legacy(stored):
    return bool(re.fullmatch(r"[0-9a-fA-F]{64}", stored or ""))
...
def get_config():
    ...
    cfg["_insecure"] = (cfg.get("api_token") == _INSECURE_TOKEN
                        or password_value(cfg) == _INSECURE_PW_SHA)
    return cfg
...
RE_SSH_USER = re.compile(r"^[A-Za-z_][A-Za-z0-9_-]{0,31}\Z")
...
RE_PW      = re.compile(r"^.{10,128}\Z", re.S)   # hdaghl 10 karaktr baraye ramz
...
def valid_port(value):
    value = str(value or "")
    return bool(RE_PORT.fullmatch(value)) and 1 <= int(value) <= 65535

def valid_ipport(value):
    value = str(value or "")
    if not RE_IPPORT.fullmatch(value):
        return False
    return valid_port(value.rsplit(":", 1)[1])
...
def build_iran_cmd(action, a):
    ...
    if action == "kcp_on":
        ...
        if not valid_port(str(port)) or not RE_PROFILE.match(str(profile)): return None
    ...
    if action == "game_add":
        ...
        if not RE_NAME.match(name) or not valid_port(inbound) or not RE_HOST.match(sni): return None
    ...
    if action == "add_node":
        ...
        if not RE_NAME.match(name) or not valid_port(inbound): return None
        cmd = ["ratholectl", "add", name, inbound]
        if api:
            if not valid_port(api): return None
            cmd += ["--api-port", api]
        return cmd
    if action == "rm_node":
        name = a.get("name", "")
    ...
    if action == "edit_node":
        ...
        if inbound:
            if not valid_port(inbound): return None
            cmd += ["--inbound", inbound]
        api = str(a.get("api_port", "") or "")
        if api:
            if api != "off" and not valid_port(api): return None
            cmd += ["--api-port", api]
    ...
    if action == "set_config":
        ...
        if key in ("fake-port", "sub-port", "control-port"):
            if not valid_port(val): return None
        elif key in ("domain",):
...
def build_node_cmd(action, a):
    ...
    if action == "plain_on":
        remote = a.get("remote", "")
        if not valid_ipport(remote): return None
    ...
    if action == "kcp_on":
        ...
        if not valid_ipport(remote) or not RE_KEY.match(key) or not RE_PROFILE.match(profile): return None
    ...
    if action == "upstream_kcp_on":
        ...
        if not RE_ID.match(uid) or not valid_ipport(remote) or not RE_KEY.match(key) or not RE_PROFILE.match(profile): return None
    ...
    if action == "set_server":
        ...
        server = a.get("server", "")
        if not (valid_ipport(server) or RE_HOST.match(server)): return None
        return ["ratholenode", "set", "SERVER", server]
    if action == "watchdog_on":
        iv = str(a.get("interval", "60") or "60")
        if not valid_port(iv): return None
    ...
    if action == "add_svc":
        ...
        if not RE_NAME.match(name) or not RE_KEY.match(token) or not valid_port(inbound): return None
    ...
    if action == "upstream_add":
        ...
        if not RE_ID.match(uid) or not valid_ipport(server): return None
    ...
    if action == "upstream_add_svc":
        ...
        if not RE_ID.match(uid) or not RE_NAME.match(name) or not RE_KEY.match(token) or not valid_port(inbound): return None
    ...
    if action == "adaptive_on":
        ...
        if not valid_port(iv) or not valid_port(fa) or not valid_port(re): return None
...
def _ssh_base(cfg, server):
    # Inventory may predate current validators or be edited manually. Validate again
    # immediately before building argv so a leading '-' can never become an ssh option.
    user = str(server.get("ssh_user", "root"))
    host = str(server.get("host", ""))
    port = str(server.get("ssh_port", 22))
    if not RE_SSH_USER.fullmatch(user) or not RE_HOST.fullmatch(host) or not valid_port(port):
        raise ValueError("invalid SSH target in inventory")
    ssh = ["ssh"] + list(cfg.get("ssh_opts", []))
    if cfg.get("ssh_key_path"):
        ssh += ["-i", cfg["ssh_key_path"]]
    ssh += ["-p", port, "%s@%s" % (user, host), "--"]
    return ssh
...
def deploy_to_server(server):
    ...
    gh = str(cfg.get("gh_repo", "ahmadmute/RatholeEngine-Pro"))
    ...
    remote = r'''set -e
GH="%s"
BASE="https://github.com/$GH/releases/latest/download"
D="$(mktemp -d)"
trap 'rm -rf "$D"' EXIT
ok=0
for M in "" "https://ghproxy.net/" "https://gh-proxy.com/" "https://mirror.ghproxy.com/"; do
  if curl -fsSL --connect-timeout 20 --retry 2 "${M}${BASE}/install.sh" -o "$D/install.sh" 2>/dev/null \
     && curl -fsSL --connect-timeout 20 --retry 2 "${M}${BASE}/SHA256SUMS" -o "$D/SHA256SUMS" 2>/dev/null; then ok=1; break; fi
done
[ "$ok" = 1 ] || { echo "download install.sh/SHA256SUMS shekast khord; update-e bedoon verify rad shod" >&2; exit 1; }
awk '$2=="install.sh" {print}' "$D/SHA256SUMS" > "$D/install.sha"
[ "$(wc -l < "$D/install.sha")" = 1 ] || { echo "checksum-e install.sh peyda nashod" >&2; exit 1; }
( cd "$D" && sha256sum -c install.sha --strict ) || { echo "checksum-e install.sh motabegh nist" >&2; exit 1; }
RATHOLE_GH="$GH" bash "$D/install.sh" --update
''' % gh
    try:
...
def provision_server(d):
    ...
    if not RE_NAME.match(name) or role not in ("iran", "node") or not RE_HOST.match(host) \
       or not RE_SSH_USER.fullmatch(user) or not valid_port(port):
...
def mock_run(server, cmd_args):
    ...
    if cmd_args[:2] == ["ratholectl", "show"] and len(cmd_args) >= 3:
        ...
        return {"rc": 0, "out":
            ...
            "curl -fsSL https://raw.githubusercontent.com/ahmadmute/RatholeEngine-Pro/main/install.sh | sudo bash -s -- --node -- \\\n"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, code, body, ctype="application/json", extra_headers=None):
        ...
        if "html" in ctype:
            self.send_header("Content-Security-Policy", "default-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; object-src 'none'; img-src 'self' data:; connect-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'")
    ...
    def _session_cookie(self, cfg, clear=False):
        secure = (self.headers.get("X-Forwarded-Proto", "").lower() == "https")
        value = "" if clear else make_session_token(cfg)
        attrs = ["rhsession=" + value, "HttpOnly", "SameSite=Strict", "Path=/"]
        if clear:
    ...
    def do_OPTIONS(self):
        return self._send(204, b"", "text/plain", {"Allow": "GET, POST, PUT, DELETE, HEAD, OPTIONS"})

    def do_HEAD(self):
        cfg = get_config()
        path = urlparse(self.path).path
        if path in ("/", "/hub/", "/hub", "/api/health", "/hub/api/health"):
    ...
    def do_POST(self):
        ...
        if p == "/api/login":
            ...
            stored = password_value(cfg)
            if verify_password(stored, pw):
                login_reset(ip)
                # migrate legacy unsalted SHA-256 without forcing a password reset.
                if password_is_legacy(stored):
                    with _lock:
                        fresh = get_config()
                        fresh["admin_password_hash"] = hash_password(pw)
                        fresh.pop("admin_password_sha256", None)
                        save_config(fresh)
                        cfg = fresh
                return self._send(200, {"ok": True},
                                  extra_headers={"Set-Cookie": self._session_cookie(cfg)})
            login_record_fail(ip)
            return self._send(401, {"error": "bad password"})

        if p == "/api/logout":
            return self._send(200, {"ok": True},
                              extra_headers={"Set-Cookie": self._session_cookie(cfg, clear=True)})
    ...
    def _config_save(self, d):
        ...
        with _lock:
            ...
            if new_pw is not None:
                cur = str(d.get("current_password", ""))
                if not verify_password(password_value(cfg), cur):
                    return self._send(403, {"error": "current password incorrect"})
                if not RE_PW.match(str(new_pw)):
                    return self._send(400, {"error": "password must be 10-128 chars"})
                cfg["admin_password_hash"] = hash_password(str(new_pw))
                cfg.pop("admin_password_sha256", None)
            ...
            if "ssh_key_path" in d:
                key_path = d["ssh_key_path"]
                if not isinstance(key_path, str) or len(key_path) > 255 or (key_path and not RE_PATH.fullmatch(key_path)):
                    return self._send(400, {"error": "ssh_key_path must be empty or an absolute safe path"})
                cfg["ssh_key_path"] = key_path; changed.append("ssh_key_path")
            if "ssh_opts" in d:
                v = d["ssh_opts"]
                if not (isinstance(v, list) and len(v) <= 30 and all(isinstance(x, str) and len(x) <= 200 and "\n" not in x and "\r" not in x and "\x00" not in x for x in v)):
                    return self._send(400, {"error": "ssh_opts must be a short list of safe strings"})
        ...
        headers = None
        if new_token:
            out["api_token"] = new_token  # one-time display for external API clients
            headers = {"Set-Cookie": self._session_cookie(cfg)}
        return self._send(200, out, extra_headers=headers)
    ...
    def _edit_server(self, name, d):
        ...
        if "ssh_user" in d:
            if not RE_SSH_USER.fullmatch(str(d["ssh_user"])): return self._send(400, {"error": "bad user"})
            changes["ssh_user"] = str(d["ssh_user"])
        if "ssh_port" in d:
            if not valid_port(str(d["ssh_port"])): return self._send(400, {"error": "bad port"})
...
<title>RatholeEngine Pro — Tunnel Control Center</title>
<style>
 ...
 *{box-sizing:border-box}
 ...
 /* ---- RatholeEngine Pro visual system ---- */
 body{background:
   radial-gradient(900px 520px at 85% -10%,rgba(37,99,235,.16),transparent 60%),
   radial-gradient(760px 480px at -10% 100%,rgba(14,165,233,.10),transparent 60%),var(--bg);
   min-height:100vh}
 #sb{background:linear-gradient(180deg,rgba(17,26,37,.98),rgba(10,15,22,.98));box-shadow:12px 0 34px rgba(0,0,0,.18)}
 #sb .logo{display:grid;grid-template-columns:30px 1fr;column-gap:9px;align-items:center;line-height:1.1;letter-spacing:-.02em}
 #sb .logo>b{grid-row:1/3;display:grid;place-items:center;width:30px;height:30px;border-radius:10px;background:linear-gradient(135deg,#2563eb,#06b6d4);color:#fff;box-shadow:0 8px 24px rgba(37,99,235,.35)}
 #sb .logo span{margin-inline-start:3px;background:linear-gradient(90deg,#60a5fa,#22d3ee);-webkit-background-clip:text;background-clip:text;color:transparent}
 #sb .logo small{display:block;color:var(--mut);font-size:9px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;margin-top:4px}
 .card,.scard,.spage-head,.spage .sec,.rcard,.gwrap{box-shadow:0 14px 36px rgba(0,0,0,.16);backdrop-filter:blur(10px)}
 .card,.scard,.spage-head,.spage .sec,.rcard{background:linear-gradient(160deg,rgba(23,34,47,.96),rgba(13,22,32,.96))}
 button{font-weight:650;box-shadow:0 5px 14px rgba(37,99,235,.14)}
 button.gh,button.s{box-shadow:none}
 input,select{outline:none;transition:border-color .18s,box-shadow .18s;background:rgba(5,11,18,.72)}
 input:focus,select:focus{border-color:#3b82f6;box-shadow:0 0 0 3px rgba(59,130,246,.14)}
 .scard{position:relative;overflow:hidden}
 .scard:before{content:'';position:absolute;inset:0 auto 0 0;width:3px;background:linear-gradient(#3b82f6,#22d3ee);opacity:.75}
 .badge{border:1px solid rgba(255,255,255,.055)}
 .toast{border-color:rgba(96,165,250,.35);box-shadow:0 18px 60px rgba(0,0,0,.65)}
 ::selection{background:rgba(59,130,246,.35);color:#fff}
 @media(max-width:720px){#sb .logo small{display:none}#sb .logo{grid-template-columns:26px auto}#sb .logo>b{width:26px;height:26px;border-radius:8px}.sitem span:not(.ic){display:none}main#page{padding:10px}.card,.spage .sec{border-radius:10px}.grid.list .scard{display:block}.grid.list .scard .top{min-width:0;margin-bottom:7px}}
</style></head><body>
<div id="app"></div>
<div class="toast" id="toast" onclick="this.classList.remove('show')"></div>
<script>
const $=id=>document.getElementById(id);
let TOKEN=sessionStorage.getItem('rh_session')||'';
let SERVERS=[];
...
const DICT={
 fa:{
  ...
  chpw:'taghir ramz modiriat',l_curpw:'ramz feli',l_newpw:'ramz jadid',pw_hint:'hadaghal 10 karakter',save_pw:'zakhire ramz',
...
async function api(m,p,b){const o={method:m,headers:{'Content-Type':'application/json'}};
 if(TOKEN&&TOKEN!=='session')o.headers['Authorization']='Bearer '+TOKEN; if(b)o.body=JSON.stringify(b);
 const r=await fetch(p,o); let j={};try{j=await r.json()}catch(e){}
 if(r.status===401){TOKEN='';sessionStorage.removeItem('rh_session');shell();} return {status:r.status,j};}
async function logout(){try{await fetch('api/logout',{method:'POST',credentials:'same-origin'});}catch(e){}TOKEN='';sessionStorage.removeItem('rh_session');shell();}
...
function shell(){
 ...
 $('app').innerHTML=`<nav id="sb">
   <div class="logo"><b>R</b> Rathole<span>Engine Pro</span><small>Control Center</small></div>
...
async function doLogin(){const {status,j}=await api('POST','api/login',{password:$('pw').value});
 if(status===200){TOKEN='session';sessionStorage.setItem('rh_session',TOKEN);if(!location.hash)location.hash='#/dashboard';shell();}else{$('msg').textContent=t('pw_wrong');}}
...
async function rotTok(){if(!confirm(t('cf_rottok')))return;const {status,j}=await api('POST','api/config',{rotate_token:true});if(status!==200){toast('error: '+(j.error||status));return;}TOKEN='session';sessionStorage.setItem('rh_session',TOKEN);toast(t('tok_applied')+'\n'+j.api_token);renderSettingsPage();}
...
async function boot(){
 try{const r=await fetch('api/servers',{credentials:'same-origin'});if(r.status===200){TOKEN='session';sessionStorage.setItem('rh_session',TOKEN);}}catch(e){}
 shell();
}
boot();
...
def main():
    ...
    if cfg.get("_insecure"):
        sys.stderr.write(
            ...
            "       api_token, admin_password_hash\n\n")
    class HubHTTPServer(ThreadingHTTPServer):
        daemon_threads = True
        allow_reuse_address = True

    httpd = HubHTTPServer((host, port), Handler)

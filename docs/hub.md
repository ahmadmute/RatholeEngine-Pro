...
```
GET    /api/health
POST   /api/login                            {"password":"..."} → {"ok":true} + HttpOnly session cookie
...
- رمز مدیریت به‌صورت `PBKDF2-SHA256` با salt تصادفی و ۳۱۰٬۰۰۰ دور ذخیره می‌شود؛ hash قدیمی SHA-256 بعد از اولین ورود موفق خودکار مهاجرت می‌کند.
- UI توکن API را در `localStorage` یا JavaScript نگه نمی‌دارد؛ نشست مرورگر فقط با کوکی `HttpOnly; SameSite=Strict` انجام می‌شود و خروج امن دارد.
- پاسخ‌ها دارای CSP، `X-Frame-Options: DENY`، `Referrer-Policy: no-referrer`، `Permissions-Policy` و `Cache-Control: no-store` هستند.
- نام کاربر SSH، host و port دوباره درست قبل از ساخت argv اعتبارسنجی می‌شوند؛ username با `-` شروع‌شونده رد می‌شود.
- آپدیت از Hub فقط وقتی اجرا می‌شود که `install.sh` با checksum منتشرشده در `SHA256SUMS` تطبیق داشته باشد.
- سرویس systemd هاب با `NoNewPrivileges`، `PrivateTmp`، محدودیت‌های kernel/filesystem و `UMask=0077` اجرا می‌شود.
